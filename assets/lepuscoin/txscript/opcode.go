...
var opcodeArray = [256]opcode{
	...
	OP_CHECKMULTISIGVERIFY: {OP_CHECKMULTISIGVERIFY, "OP_CHECKMULTISIGVERIFY", 1, nil},
...
func opcodeCheckLockTimeVerify(op *parsedOpcode, vm *Engine) error {
	...
	// type.
	if !((uint32(vm.tx.LockTime) < LockTimeThreshold && int64(lockTime) < int64(LockTimeThreshold)) ||
		(uint32(vm.tx.LockTime) >= LockTimeThreshold && int64(lockTime) >= int64(LockTimeThreshold))) {
